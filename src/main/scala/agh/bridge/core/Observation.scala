...
object PlayerObservation:
    ...
    object Bidding:
        def apply(py: Dynamic): Bidding =
          ...
          Bidding(
            ...
            bid = Bid.fromOptional(dict("bid")),
            declarer = PlayerDirection.fromOptional(dict("declarer")),
            multiplier = dict("multiplier").as[Int],
          )

    final case class Game(
      roundPlayer: Option[PlayerDirection],
    ...
    object Game:
        def apply(py: Dynamic): Game =
          val dict = py.as[Map[String, Dynamic]]
          Game(
            roundPlayer = PlayerDirection.fromOptional(dict("round_player")),
